...
class JobShopScheduler:
    ...
    def _add_share_machine_constraint(self):
        ...
        while head < len(sorted_tasks):
            ...
            for i,task in enumerate(same_machine_tasks):
    ...
    def get_bqm(self, stitch_kwargs=None):
        ...
        t_one_start = end - start

        start = time.time()
        self._add_precedence_constraint()
        end = time.time()
        t_precedence = end - start

        start = time.time()
        self._add_share_machine_constraint()
        end = time.time()
        t_share_machine = end - start

        start = time.time()
        self._remove_absurd_times()
        end = time.time()
        t_remove_absurd_times = end - start

        # Get BQM
        start = time.time()
        bqm = dwavebinarycsp.stitch(self.csp, **stitch_kwargs)
        end = time.time()
        t_stitcher = end - start

        start = time.time()
        # Edit BQM to encourage an optimal schedule
        self._edit_bqm_for_shortest_schedule(bqm)
        end = time.time()
        t_edit_bqm= end - start

        return bqm, t_one_start, t_precedence, t_share_machine, t_remove_absurd_times, t_stitcher,t_edit_bqm

def compute_max_time(job_list):
    max_time = 0  # Upperbound on how long the schedule can be
    for job in list(job_list.values()):
        for process in job:
            t = process[1]
            max_time+=t
    return max_time
